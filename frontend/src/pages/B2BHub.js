...
export default function B2BHub() {
  ...
  return (
    <div data-testid="b2b-hub-page">
      <Helmet>
        <title>{`B2B Innovation Hub | ${COMPANY.name}`}</title>
        <meta name="description" content="Partner with Quantum Medical for hospital-grade prosthetics & orthotics digital workflows, fast in-house fabrication, and dedicated clinical support." />
      </Helmet>
        ...
        <div className="mx-auto grid max-w-7xl grid-cols-1 items-start gap-14 px-5 lg:grid-cols-2 lg:px-8">
          ...
          <Reveal delay={0.1}>
            <form onSubmit={submit} className="space-y-4 rounded-3xl border border-[#E2E8F0] bg-[#F8F9FA] p-7 sm:p-9" data-testid="b2b-form">
              ...
              <button type="submit" disabled={loading} data-testid="b2b-submit-btn" className="flex w-full items-center justify-center gap-2 rounded-full bg-[#FF6B4A] py-3.5 font-display text-sm font-semibold text-white transition-colors hover:bg-[#e8532f] disabled:opacity-60">
                {loading ? <Loader2 size={17} className="animate-spin" /> : <Send size={16} />} {loading ? "Sending…" : "Request Partnership"}
              </button>
            </form>
          </Reveal>
        </div>
